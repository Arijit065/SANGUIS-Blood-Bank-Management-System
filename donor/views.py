...
def donor_dashboard_view(request):
    ...
    demand = bmodels.BloodRequest.objects.values('bloodgroup').annotate(count=Count('id')).order_by('-count')
    demand_groups = [d['bloodgroup'] for d in demand] if demand else ['A+', 'O+', 'B+', 'AB+']
    demand_counts = [d['count'] for d in demand] if demand else [0, 0, 0, 0]
    ...
    dict={
        ...
        'demand_groups': json.dumps(demand_groups),
