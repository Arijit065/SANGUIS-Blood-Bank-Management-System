...
def admin_dashboard_view(request):
    ...
    donations = dmodels.BloodDonate.objects.filter(status='Approved').values('date').annotate(count=Count('id')).order_by('date')
    donation_dates = [d['date'].strftime("%b %d") for d in donations][-10:] if donations else []
    donation_counts = [d['count'] for d in donations][-10:] if donations else []
    ...
    dict={
        ...
        'donation_dates': json.dumps(donation_dates),
